...
if [ "$DRY" = "true" ]; then
  "$PY" "{{cookiecutter.project_slug}}/tools/transformers/translator.py" techstack apply --dry-run "$TS_PATH"
else
  "$PY" "{{cookiecutter.project_slug}}/tools/transformers/translator.py" techstack apply "$TS_PATH"
fi
