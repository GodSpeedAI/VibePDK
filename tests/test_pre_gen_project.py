...
def run_pre_gen_script(
    monkeypatch: Any, slug: str, author: str, pyver: str, description: str
) -> Tuple[Mock, Mock]:
    """Helper to run the pre-gen script with patched globals.

    Note: `monkeypatch` is provided by pytest; we don't use it directly here but
    keep it in the signature for parity with test functions.
    """
    ...
    script = (
        PRE_GEN_SCRIPT.replace("{{ cookiecutter.project_slug }}", slug)
        .replace("{{ cookiecutter.author_name }}", author)
        .replace("{{ cookiecutter.python_version }}", pyver)
        .replace("{{ cookiecutter.description }}", description)
    )
...
def test_project_slug_valid(monkeypatch: Any, slug: str) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
...
def test_project_slug_invalid(monkeypatch: Any, slug: str) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
        monkeypatch, slug, "author", "3.11", "description"
    )
...
def test_author_name_valid(monkeypatch: Any) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
        monkeypatch, "slug", "author", "3.11", "description"
    )
    mock_exit.assert_not_called()
...
def test_author_name_invalid(monkeypatch: Any) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
...
def test_python_version_valid(monkeypatch: Any, pyver: str) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
...
def test_python_version_invalid(monkeypatch: Any, pyver: str) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
        monkeypatch, "slug", "author", pyver, "description"
    )
    mock_exit.assert_called_once_with(1)
...
def test_description_valid(monkeypatch: Any) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
...
def test_description_invalid(monkeypatch: Any) -> None:
    _mock_print, mock_exit = run_pre_gen_script(
